...
#     $env:FLASK_APP = "flask-pipenv-v5.py"
...
@app.route("/")
def home():
    # passing cookies to the home page
    return render_template('home-v5.html', codes = session.keys())
...
def your_url():
    if request.method == 'POST':
        ...
        if 'url' in request.form.keys():
            ...
            f.save(os.path.join(app.root_path, 'static/user_files/' + full_name))
        ...
        with open('urls.json', 'w') as url_file:
            ...
            # saves the code into the cookie
            session[request.form['code']] = True

        # args is for querystring
        # form is for post
        return render_template('your_url-v5.html', code = request.form['code'] )
    else:
...
def page_not_found(error):
    return render_template('page_not_found-v5.html'), 404
